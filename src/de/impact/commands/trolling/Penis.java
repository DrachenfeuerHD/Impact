...
public class Penis extends Command {
    ...
    public void onChat(String[] aliases, Player p) {
        ...
        // Bottom
        for(var x = target.getLocation().getX() - 4; x <= target.getLocation().getX() + 4; x++)
            for(var y = target.getLocation().getY() + 4; y <= target.getLocation().getY() + 6; y++)
                for(var z = target.getLocation().getZ(); z <= target.getLocation().getZ() + 2; z++)
                    target.sendBlockChange(new Location(target.getWorld(), x, y, z), Material.WOOL, (byte) 0);

        // Stem
        for(var x = target.getLocation().getX() - 1; x <= target.getLocation().getX() + 1; x++)
            for(var y = target.getLocation().getY() + 6; y <= target.getLocation().getY() + 12; y++)
                for(var z = target.getLocation().getZ(); z <= target.getLocation().getZ() + 2; z++)
                    target.sendBlockChange(new Location(target.getWorld(),x,y,z), Material.WOOL, (byte) 0);

        // Top
        for(var x = target.getLocation().getX() - 1; x <= target.getLocation().getX() + 1; x++)
            for(var y = target.getLocation().getY() + 12; y <= target.getLocation().getY() + 14; y++)
                for(var z = target.getLocation().getZ(); z <= target.getLocation().getZ() + 2; z++)

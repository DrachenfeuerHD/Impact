...
public class Gamemode extends Command {
    ...
    public void onChat(String[] aliases, Player p) {
        ...
        switch (aliases[0]) {
            case "0": changeGameMode(target, p, GameMode.SURVIVAL);
            case "1": changeGameMode(target, p, GameMode.CREATIVE);
            case "2": changeGameMode(target, p, GameMode.ADVENTURE);
            case "3": changeGameMode(target, p, GameMode.SPECTATOR);
            default: ChatUtils.sendMessage(p, "Please use " + getUsage());
        }
    }
